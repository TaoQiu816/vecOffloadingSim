...
FINAL_LOG="${FINAL_LOG_PATH:-${ROOT_DIR}/runs/rc1_ablation_finalize_$(date '+%Y%m%d_%H%M%S').log}"
...
SCRIPT_PATHS=(
  "scripts/ablation/run_rc1_ablation_screen_job.sh"
  "scripts/ablation/finalize_rc1_ablation_runs.sh"
  "scripts/ablation/launch_rc1_ablation_screens.sh"
  "diagnostics/ablation_plan/MODULE_MAPPING.md"
  "diagnostics/ablation_plan/ABLATION_MODEL_DEFINITIONS.md"
  "diagnostics/ablation_plan/CENTRALIZED_PPO_FEASIBILITY.md"
)
...
for rel_run_dir in "${RUN_REL_PATHS[@]}"; do
  while IFS= read -r rel_log; do
    [[ -n "${rel_log}" ]] || continue
    git reset -q HEAD -- "${rel_log}" || true
  done < <(find "${rel_run_dir}" -type f -name '*.log' -print)
done
