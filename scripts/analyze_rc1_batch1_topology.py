...
BASE_MAIN_RUN = ROOT / "runs" / "run_1000ep_A_20260320"
...
def build_ippo_main_summary() -> pd.DataFrame:
    ...
    mappo = _tail_mean(BASE_MAIN_RUN / "logs" / "training_stats.csv", n=100)
    mappo.update(
        {
            "run_name": "mappo_main_A",
            "label": "MAPPO Main A (base scene)",
        }
    )
    rows.append(mappo)
    return pd.DataFrame(rows)
...
def plot_batch_overview(topology_df: pd.DataFrame, ippo_df: pd.DataFrame) -> None:
    ...
    for ax, (metric, title) in zip(axes.flat, ippo_metrics):
        ...
        ax.grid(axis="y", alpha=0.25)
    fig.suptitle("Base Scene: IPPO Main vs MAPPO Main A (tail100)", fontsize=15)
    fig.savefig(OUT_DIR / "batch1_ippo_main_vs_mappo.png", dpi=180)
    plt.close(fig)


def write_report(topology_df: pd.DataFrame, ippo_df: pd.DataFrame, space_df: pd.DataFrame) -> None:
    ...
    ippo = ippo_df[ippo_df["run_name"] == "ippo_main"].iloc[0]
    mappo = ippo_df[ippo_df["run_name"] == "mappo_main_A"].iloc[0]
    lines.append(
        f"- Base scene: `IPPO` tail100 vs `MAPPO A` tail100 -> "
        f"`task_sr {ippo['task_sr']:.4f} vs {mappo['task_sr']:.4f}`, "
        f"`deadline_miss {ippo['deadline_miss_rate']:.4f} vs {mappo['deadline_miss_rate']:.4f}`, "
        f"`mean_cft {ippo['mean_cft_completed']:.4f} vs {mappo['mean_cft_completed']:.4f}`."
    )
...
def main() -> None:
    ...
    write_report(topology_df, ippo_df, space_df)
