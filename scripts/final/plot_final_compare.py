...
def _plot_with_baselines(
    ...
    x = df_rl[x_col]
    y = df_rl[y_col_rl] if y_col_rl in df_rl.columns else None
    if y is not None:
        yy = _rolling(y, window)
        if percent:
            yy = yy * 100.0
        plt.plot(x, yy, label="Ours (MAPPO)", linewidth=2.2, color="#2563eb")
    for policy in df_b["policy"].unique():
        d = df_b[df_b["policy"] == policy]
        if y_col_b not in d.columns:
            continue
        yb = _rolling(d[y_col_b], window)
        if percent:
            yb = yb * 100.0
        plt.plot(d[x_col], yb, label=policy, linewidth=1.6, linestyle="--", alpha=0.9)
    plt.title(title)
    plt.xlabel("Episode")
    plt.ylabel(ylabel)
...
def main():
    ...
    # Save a summary table: RL means + baseline policy means.
