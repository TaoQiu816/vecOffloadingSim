...
def _plot_metric(df: pd.DataFrame, metric: str, title: str, ylabel: str) -> Path:
    ...
    for key in SERIES_KEYS:
        low = pd.to_numeric(df[f"{metric}__{key}__band_low"], errors="coerce")
        high = pd.to_numeric(df[f"{metric}__{key}__band_high"], errors="coerce")
        smooth = pd.to_numeric(df[f"{metric}__{key}__smooth"], errors="coerce")
        ax.fill_between(df["episode"], low, high, color=PALETTE[key], alpha=0.11, linewidth=0)
