    ...
    def save_model(self, agent, episode, is_best=False):
        ...
        try:
            checkpoint = {
                ...
                'power_log_std': agent.power_log_std.data
            }
        ...
        try:
            ...
            if 'loss' in df.columns:
                save_plot(episodes, {'Loss': df['loss']},
                          'Training Loss', 'Loss Value', 'loss_curve.png')
            ...
            if offload_data:
                save_plot(episodes, offload_data, 'Decision Distribution', 'Proportion (%)', 'offloading_ratio.png')

            # (E) [新增] 多智能体公平性指标 (Jain's Fairness Index)
            # 衡量不同车辆间奖励分配的均匀程度，越接近1越公平
            if 'ma_fairness' in df.columns:
                save_plot(episodes, {'Fairness Index': df['ma_fairness']},
                          'System Fairness (Jain Index)', 'Fairness [0-1]', 'ma_fairness.png')

            # (F) [新增] 协作活跃度 (V2V Collaboration Rate)
            # 展示智能体之间通过 V2V 进行协作的频率变化
            ...
            # (G) [新增] 奖励差距 (Reward Gap)
            # 记录群体中最大奖励与最小奖励的差值，反映贫富差距
            ...
            # (H) 队列拥堵情况 (Tasks)
            queue_data = {}
            if 'avg_veh_queue' in df.columns: queue_data['Vehicle'] = df['avg_veh_queue']
            if 'avg_rsu_queue' in df.columns: queue_data['RSU'] = df['avg_rsu_queue']
            if queue_data:
                save_plot(episodes, queue_data, 'Average Queue Length', 'Tasks', 'queue_len.png')
